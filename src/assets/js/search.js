...
const closeSearch = document.querySelector('.close-ico');
search.addEventListener('input', searchInputEv);
search.addEventListener('focus', focusInput)
closeSearch.addEventListener('click', removeFocus);
var stamp = 0;
...
function searchInputEv() {
  ...
  if (searchString.length >= 3) {
    ...
    try {
      ...
      setTimeout(function() {
        // console.log(stampT, stamp, searchString);
        var r = searchFor(searchString, stampT);
        if (stamp == r.hash) {
          renderMovieList(r.toHide, r.toShow);
        } else {
          // console.log(stampT, r.hash, '%cnie rysuje', 'color:red');
        }
      }, 500);
    }
  } else {
...
function searchFor(searchString, hash) {
  ...
  for (let i in movieList) {
    ...
    if (movieList[i].querySelector) {
      let title = movieList[i].querySelector('.movie__title').innerText;
      // console.log(title);
      if (
        ...
        toHide.push(movieList[i]);
      } else {
        toShow.push(movieList[i]);
      }
    }
  }
  var r = { hash: hash, toHide: toHide, toShow: toShow };
  return r;
}
